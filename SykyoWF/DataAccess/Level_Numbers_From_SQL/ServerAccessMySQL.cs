...
namespace WindowsFormsApplication1.BL
{
    public class ServerAccessMySQL : ServerAccessInterface
    {
        ...
        public ServerAccessMySQL()
        {
            ...
            {
                mySQLConnection = getMySQLConnection("rpro0001_cabinet");
            }
        ...
        public List<int[]> GetContent(int[] r, int[] aMul, int[] aLoc, int limit)
        {
            ...
            try
            {
                ...
                if (r[0] == 0 && r[1] == 0)
                {
                    commandString = "SELECT `e`, `f` FROM `" + DB + "`.`s" + aLoc[0].ToString() + "` WHERE `a` = " + aLoc[0] + " AND `b` = " + aLoc[1] + " AND `c` = " + r[0] + " AND `d` = " + r[1] + " ";
                }
                else
                {
                    commandString = "SELECT `e`, `f` FROM `" + DB + "`.`s" + aLoc[0].ToString() + "` WHERE `a` = " + aLoc[0] + " AND `b` = " + aLoc[1] + " ";
                }
