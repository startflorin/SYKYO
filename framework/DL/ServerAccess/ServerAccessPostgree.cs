...
{
    public class ServerAccessPostgree// : ServerAccessInterface
    {
        #region Variables
        ...
        public List<int[]> CreateSymbolAlias(string SymbolAliasName, int ExistentID)
        {
            ...
            if (AcceptSymbols)
            {
                try
                {
                    ...
                    NpgsqlCommand mySqlCommand = new NpgsqlCommand(commandString, NpgsqlConnection);
                    mySqlCommand.ExecuteNonQuery();
                    if (PostgreeTrace) SQLView.Log(mySqlCommand.CommandText);
                    /*if (NpgsqlCommand.LastInsertedId > 0)
                    {
                        symbolPosition[0] = (int)NpgsqlCommand.LastInsertedId;
                        ResultedSymbols.Add(new int[] { ExistentID, 0 });
                    }*/
                }
                catch (NpgsqlException retrieveSymbolIndexException)
