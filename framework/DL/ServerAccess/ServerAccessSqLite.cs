...
{
    public class ServerAccessSqLite// : ServerAccessInterface
    {
        #region Variables
        ...
        public List<int[]> CreateSymbolAlias(string SymbolAliasName, int ExistentID)
        {
            ...
            if (AcceptSymbols)
            {
                try
                {
                    ...
                    //if (sqlitet) SQLView.Log(SqLiteCommand.CommandText);
                    /*if (mySqlCommand.LastInsertedId > 0)
                    {
                        symbolPosition[0] = (int)SQLiteCommand.LastInsertedId;
                        ResultedSymbols.Add(new int[] { ExistentID, 0 });
                    }*/
                }
                catch (SQLiteException retrieveSymbolIndexException)
        ...
        public List<int[]> CreateSymbolByName(string SymbolName, bool AcceptDuplicates)
        {
            ...
            try
            {
                ...
                if (SqLiteConnection.LastInsertRowId > 0)
                {
                    ...
                    symbolPosition[0] = (int)SqLiteConnection.LastInsertRowId;
        ...
        {
            string commandString = "CREATE TABLE IF NOT EXISTS `" + DB + "`.`s" + symbolPosition[0].ToString("00000000") + "` ( `ID` INT , `a` INT NULL , `b` INT NULL , `c` INT NULL , `d` INT NULL , `e` INT NULL , `f` INT NULL , `g` INT NULL , `h` INT NULL , PRIMARY KEY (`ID`) );";
